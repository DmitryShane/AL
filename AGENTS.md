...
After any Unity Activity Logger plugin change under `unity_al`, bump the Unity editor plugin version in `unity_al/Editor/UnityAL.cs` (`PluginVersion`) only. For fixes and small corrections, increment the third number in the current `0.2.X` track. For logic updates or behavior changes, increment the second number and reset the third number to `0` (for example, `0.2.7` -> `0.3.0`). Do **not** bump `com.mempic.al/package.json` or the Unity project `Packages/manifest.json` tag for Unity AL runtime changes unless the owner explicitly asks to publish/update the root Unity package tag.
...
- Do not restart or start the local app after backend changes by default. Run `stop`/`start` only when the owner explicitly requests it or when a local runtime verification genuinely requires it; in that case, verify `http://127.0.0.1:8000/api/v1/health` after startup.
...
- When the owner asks to **analyze**, **investigate**, **check**, or **figure out** a problem, do diagnostic work only: inspect the relevant code/data/production state, identify the root cause, and report the findings clearly. Do **not** change code, data, configuration, or production state until the owner gives a separate explicit command to implement/fix/change it.
...
- Do not `source /etc/al/backend.env`; it is not guaranteed to be shell-source compatible. If individual env values are needed for diagnostics, parse them deliberately without printing secrets.
...
- To start a scoped production Activity rebuild, use the running backend's local HTTP API from the server, not Python router imports, direct helper calls, or manual Mongo job insertion. The standard flow is: SSH to `root@activity.mempic.com`, create a temporary cookie file, call `POST http://127.0.0.1:8000/api/v1/auth/dev-login` to get a local admin session, then call the scoped rebuild endpoint, for example `POST "http://127.0.0.1:8000/api/v1/authors/Denis%20Ostrovskiy/activity/rebuild?startDate=2026-06-16&endDate=2026-06-16"`. Remove the cookie file afterwards.
- Do not inspect production Python signatures or instantiate `BackendContainer` just to launch a rebuild. The local HTTP API path is the source of truth because it exercises the same auth, lock, background-task, progress, and diagnostics behavior as the website.
- For production rebuild monitoring, prefer the public status endpoint when an authenticated cookie is already available; otherwise use read-only `mongosh` inspection of `aggregate_rebuild_jobs` by `jobId`. Poll until `status` is `completed` or `failed`, then report `rawPhaseWallSeconds`, `rawEventsPerSecond`, `rawAccumulatorHandledEventsByType`, `rawLegacyFallbackEventsByType`, `rawAccumulatorFallbackReasonsByType`, `memoryGuardPauses`, `rssPeakMb`, and any `error`.
